...
with tabs[0]:
    ...
    if not bundle:
        ...
        if macro_risk.blocked:
            final="NO TRADE"
        elif macro_risk.level in ("HIGH","MEDIUM") and final not in ("WAIT","NO TRADE"):
            final="WAIT"
        ...
        if bundle["score"]>=75 and final not in ("WAIT","NO TRADE"):
            st.success("🚨 SETUP ALERT — confluencia alta. Preparar plan, no perseguir precio.")
        elif final=="NO TRADE":
...
with tabs[3]:
    ...
    if not bundle:
        ...
        if candidate.startswith("BUY"):
            direction="BUY"
        elif candidate.startswith("SELL"):
            direction="SELL"
        else:
            direction=None
...
- R:R preferente ≥ 1:3.
- Score ≥ 75 = alerta de calidad, no garantía.
